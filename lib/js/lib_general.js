...
$(document).ready(function() {
	...
	settingsLoad(function(settings){
		$settings = settings;
		/* load the logo  - fill all <div class="logo"> with content */
		$(".logo").html('<img id="logo" src="'+$settings["settings_platform_logo"]+'" style="width:200px;"/>');
		$(".navbar-brand img").attr('src',"images/dataworx_icon.ico");
		$(".navbar-brand img").css('height',"18px");
		// all functions that rely on the server settings beeing loaded need to bind to this event like this:
		/*
...
function settingsLoad(ResultHandler) {
	
	var url = "lib/php/lib_settings.php";
	var data = null;
	submitUrl(url,data,ResultHandler);
}
...
function goToPage(url)
{
	...
	if($settings["settings_debug_mode"] == true)
	{
