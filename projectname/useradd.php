...
if(!empty($_REQUEST['activation']))
{
	...
	if(strlen($received_activation)>0)
	{
		...
		// http://localhost/PDT/LOGIN_PHP/useradd.php?activation=ef0521b52aaefa764753e84759dc5910
...
else if(!empty($_REQUEST['username']) && !empty($_REQUEST['password'])) /* is it an registration? */
{
	...
	if ($requested_password_confirm != $requested_password) {
		...
		if(!$output)
		{
			...
			$text = $settings_email_activation_text." Please click : <a href='login.php?activation=".$activation."'>this link to activate your account</a>";
...
<div data-role="page" id="useradd">
	...
	?>
		<form id="useraddForm" class="useraddForm" action="useradd.php" method="post" accept-charset="UTF-8">
