...
/* is it an activation ? */
$received_activation = "";
require('../library/lib_general.php');
...
if(!empty($_REQUEST['activation']))
{
	...
	if(strlen($received_activation)>0)
	{
		...
		// http://localhost/PDT/LOGIN_PHP/backend_useradd.php?activation=ef0521b52aaefa764753e84759dc5910
...
else if(!empty($_REQUEST['username']) && !empty($_REQUEST['password'])) /* is it an registration? */
{
	...
	if ($requested_password_confirm != $requested_password) {
		...
		if(!$output)
		{
			...
			$text = $settings_email_activation_text." Please click : <a href='frontend_frontend_login.php?activation=".$activation."'>this link to activate your account</a>";
...
<div data-role="page" id="useradd">
	...
	<?php
		...
		require ("../library/lib_upload.php");
	?>
		<form id="useraddForm" class="useraddForm" action="backend_useradd.php" method="post" accept-charset="UTF-8">
